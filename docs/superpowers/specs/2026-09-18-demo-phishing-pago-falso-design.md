...
1. **Ningún dato real.** El demo solo usa datos ficticios de prueba: tarjeta `4242 4242 4242 4242`, titular ficticio, emails y teléfonos de prueba.
2. **Secrets solo en `.env.local`** (gitignored). `TELEGRAM_BOT_TOKEN` y `TELEGRAM_CHAT_ID` jamás van en código fuente ni en repositorio.
3. **Banner de entorno simulado siempre visible** en la página principal, advirtiendo que es una demo educativa y que no se deben ingresar datos reales.
4. **Sin dependencias externas innecesarias:** no hay servidor Redis (se usa un mock en memoria), y las rutas de BIN (`lookup`, `luhn`, `validate`) son simuladas (sin llamadas a `lookup.binlist.net`).
5. **Exposición mínima:** el webhook de Telegram solo es alcanzable durante la presentación a través de un túnel HTTPS (ngrok/cloudflared) y con el webhook registrado bajo demanda.
...
- Sin otras dependencias de terceros: el mock de Redis, el cliente de Telegram y las utilidades BIN simuladas son código propio.
...
└── src/
    ...
    │       │   ├── savePaymentState/route.ts # persiste estado en mock redis
...
1. La página demo genera un `sessionId` fresco (p. ej. `demo-<timestamp>`) y precarga datos de prueba en `localStorage` (`checkout_payment`).
...
5. El webhook lee el `Session ID` del texto del mensaje, guarda el nuevo estado en el mock redis y edita el mensaje en Telegram (p. ej. "📌 ESTADO: SE SOLICITÓ OTP"), respondiendo el callback.
6. En el siguiente poll, el modal detecta el estado `otp` y muestra el input correspondiente.
7. El participante (voluntario) escribe el OTP de prueba; el modal lo publica en el chat del bot.
...
- **`utils/paymentStorage.ts`**: intacto, funciona contra el mock de redis.
...
- **Página `page.tsx`**: incluye el banner, un título/contexto ("Así funciona un ataque de pago falso"), un botón que abre el modal (con props y datos de prueba) y una guía breve del presentador (pasos en Telegram). No se replica el chat del atacante en pantalla: el presentador proyecta la app de Telegram real.
...
| `POST /api/telegram/savePaymentState` | Mock | Persiste `{sessionId, status}` en mock redis. |
| `GET /api/telegram/checkStatus` | Mock | Lee el estado de la sesión del mock redis (devuelve `loading` si no existe). |
| `POST /api/telegram/sendMessageLogs` | Real | Publica logs en el chat (mensaje simple). |
| `POST /api/telegram/webhook` | Real (recepción) | Procesa `callback_query`: mapea `data` → estado, guarda en mock redis, edita el mensaje en Telegram, responde el callback. |
...
- Documentación en `README.md`: pasos para correr (`npm run dev`), levantar el túnel (`ngrok http 3000` o `cloudflared tunnel --url http://localhost:3000`), registrar el webhook y limpiarlo.

## Verificación

1. `npm run build` y `npm run lint` sin errores.
2. Prueba manual del bucle completo: abrir la página, click en el botón, verificar que el modal publica el mensaje en el chat del bot, pulsar "Pedir OTP" en Telegram, verificar que el modal cambia al estado OTP, ingresar OTP de prueba, pulsar "✅ Check", verificar "PAGO APROBADO".
3. Verificar que no hay secrets en el código fuente ni en git.
